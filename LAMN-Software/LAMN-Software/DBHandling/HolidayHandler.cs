...
namespace LAMN_Software
{
    public class HolidayHandler
    {
        ...
        public Exception GetAllHolidaysForWeek(int weekNmr)
        {
            allHolidayRequests = new List<Holiday>();
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(connStr))
                {
                    string sql = "SELECT * FROM holidays WHERE weekNumber = @weekNmr AND Approved = @approved";
                    MySqlCommand cmd = new MySqlCommand(sql, conn);
                    conn.Open();

                    cmd.Parameters.AddWithValue("@weekNmr", weekNmr.ToString());
